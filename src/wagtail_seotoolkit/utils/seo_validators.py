"""
Reusable SEO validation utilities.
...
def get_validation_summary(validation_result: Dict[str, any]) -> str:
    ...
    medium_severity_count = sum(1 for issue in validation_result["issues"] if issue["severity"] == "medium")
    low_severity_count = sum(1 for issue in validation_result["issues"] if issue["severity"] == "low")
...
def batch_validate_metadata(metadata_list: List[Dict[str, str]], metadata_type: str) -> List[Dict[str, any]]:
    """
    ...
    for item in metadata_list:
        ...
        if metadata_type == "title":
            ...
            validation = {
                "is_valid": False,
                "issues": [{"type": "invalid_type", "severity": "high", "message": "Invalid metadata type"}]
            }
