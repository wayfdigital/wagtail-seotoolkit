...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        scheduled_audit = (
            SEOAuditRun.objects.filter(status="scheduled")
            .order_by("created_at")
            .first()
        )

        if not scheduled_audit:
            self.stdout.write(self.style.SUCCESS("No scheduled audits found."))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Scheduled audit failed: {str(e)}"))
