...
from wagtail_seotoolkit.core.models import SEOAuditIssueSeverity
from wagtail_seotoolkit.core.utils.checkers import (
    ContentChecker,
    FreshnessChecker,
    HeaderChecker,
    ImageChecker,
...
def run_audit_on_pages(
    ...
    overall_score = calculate_audit_score(
        high_issues, medium_issues, low_issues, total_pages
    )
...
def _run_pagespeed_only_check(page, debug: bool = False) -> List[Dict[str, Any]]:
    ...
    """
    from wagtail_seotoolkit.core.utils.checkers.pagespeed_checker import (
        PageSpeedChecker,
    )
...
def calculate_audit_score(
    high_issues: int, medium_issues: int, low_issues: int, total_pages: int
) -> int:
    """
