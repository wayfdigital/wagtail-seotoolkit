...
class SEOTitle(models.Model):
    """
    Stores SEO title overrides for pages with history tracking.
    The active title will be used instead of the page's default title in the rendered HTML.
    """

    page = models.ForeignKey(
        "wagtailcore.Page",
        on_delete=models.CASCADE,
        related_name="seo_titles",
        help_text="The page this SEO title applies to",
    )
    title = models.CharField(
        max_length=255,
        help_text="The SEO title that will replace the page's default title tag",
    )
    is_active = models.BooleanField(
        default=True,
        help_text="Whether this title is currently active. Only one title can be active per page.",
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.page.title} - {self.title[:50]}"

    def save(self, *args, **kwargs):
        # If this is being set as active, deactivate all other titles for this page
        if self.is_active:
            SEOTitle.objects.filter(page=self.page, is_active=True).exclude(
                pk=self.pk
            ).update(is_active=False)
        super().save(*args, **kwargs)


class SEOMetaDescription(models.Model):
    """
    Stores SEO meta description overrides for pages with history tracking.
    The active description will be used instead of the page's default meta description in the rendered HTML.
    """

    page = models.ForeignKey(
        "wagtailcore.Page",
        on_delete=models.CASCADE,
        related_name="seo_meta_descriptions",
        help_text="The page this SEO meta description applies to",
    )
    description = models.TextField(
        max_length=320,
        help_text="The SEO meta description that will replace the page's default meta description",
    )
    is_active = models.BooleanField(
        default=True,
        help_text="Whether this description is currently active. Only one description can be active per page.",
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return f"{self.page.title} - {self.description[:50]}"

    def save(self, *args, **kwargs):
        # If this is being set as active, deactivate all other descriptions for this page
        if self.is_active:
            SEOMetaDescription.objects.filter(page=self.page, is_active=True).exclude(
                pk=self.pk
            ).update(is_active=False)
        super().save(*args, **kwargs)
